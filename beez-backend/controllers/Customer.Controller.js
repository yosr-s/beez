...
const CustomerController = {
  create: function (req, res) {
    ...
    CustomerModel.create(req.body, function (err, item) {
      ...
      res
        .status(200)
        .json({ status: 200, message: "created Customer", data: item });
    });
  },
  read: function (req, res) {
    CustomerModel.find({}, function (err, items) {
      if (err) {
        ...
        res
          .status(200)
          .json({ status: 200, message: "created Customer", data: items });
      }
    })
      .select("-__v")
      .populate("orders", "-__v");
  ...
  update: function (req, res) {
    ...
    if (!req.file) {
      return res
        .status(400)
        .json({ status: 400, message: "No photo provided", data: null });
    }
